...
x_coords <- seq(from = x_min, to = x_max, by = cell_size)
y_coords <- seq(from = y_min, to = y_max, by = cell_size)

# Create a mesh grid of coordinates
grid <- expand.grid(x = x_coords, y = y_coords)

# Convert the grid to an sf object
grid_sf <- st_as_sf(grid, coords = c("x", "y"), crs = 4326)  # Change crs according to your data
...
# Define the desired area for each grid cell in square kilometers
cell_area_km2 <- 100

# Calculate the number of cells needed along x and y to achieve the desired area
x_extent_km <- 100  # Adjust according to your study area
y_extent_km <- 100  # Adjust according to your study area

# Calculate the number of cells needed along x and y axes
num_cells_x <- sqrt(x_extent_km * y_extent_km / cell_area_km2)
num_cells_y <- num_cells_x  # Assuming square grid cells, adjust if needed

# Calculate cell size based on the number of cells and extent
cell_size <- x_extent_km / num_cells_x

# Create a grid of x and y coordinates
x_coords <- seq(from = 0, by = cell_size, length.out = num_cells_x)
y_coords <- seq(from = 0, by = cell_size, length.out = num_cells_y)
...
# Define the study area extent using longitude and latitude
study_extent <- st_bbox(c(xmin = -10, xmax = 10, ymin = 40, ymax = 60), crs = 4326)

# Calculate the cell size based on the desired area
cell_area_km2 <- 100
cell_size <- sqrt(cell_area_km2) * 1000  # Convert from km to meters

# Create the grid of square cells
grid_sf <- st_make_grid(study_extent, cellsize = cell_size, what = "polygons")

# Reshape the grid into a 10x10 matrix of cells
grid_matrix <- matrix(grid_sf, nrow = 10, byrow = TRUE)
...
# # 100km2 grids of Czech Republic
 ...
 CZ_grids_samp <- CZ_grids[c(1:3,5),]
 
 qtm(CZ_grids[c(1:3,5),])
 qtm(CZ_grids_samp)
# 
