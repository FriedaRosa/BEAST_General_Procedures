...
levels_data <- unique(presence_data$cell_grouping)
desired_levels_data <- factor(levels_data, 
                         ordered = T, 
                         levels = c("1", "2", 
                                    "4", "8", 
                                    "16", "32", 
...
species_data$cell_grouping <- as.numeric(as.character(species_data$cell_grouping))
...
length(unique(sp_3scales_tp1)) # 172 species
length(unique(sp_3scales_tp2)) # 147 species
length(unique(sp_3scales_tp3)) # 198 species
...
log_ratio_data <- species_data_wide %>% 
  mutate(log_R2_1 = log(tp2_AOO_1/tp1_AOO_1), # for tp 1-2 comparison
         log_R3_2 = log(tp3_AOO_1/tp2_AOO_1), # for tp 2-3 comparison
         log_R3_1 = log(tp3_AOO_1/tp1_AOO_1), # for tp 1-3 comparison
         .before = 1) %>% # sort columns to the beginning of the table
  select(verbatim_name, log_R2_1, log_R3_2, log_R3_1)

species_data_new3 <- merge(species_data_new2, log_ratio_data, by="verbatim_name")
species_data_new3 %>% write.csv(file=paste0(out_path, "Big_table.csv"))
