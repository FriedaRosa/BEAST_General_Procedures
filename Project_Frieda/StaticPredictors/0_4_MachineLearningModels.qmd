...
cols_all <-  c("tp", "dataset", "verbatim_name", "log_R2_1", # General ID cols
               ...
               "maxDist_toBorder", "sp_COG_long", "sp_COG_lat", "Southernness", "Westernness", "Dist_toCOD", 
               "atlas_nsDist", "atlas_ewDist", "atlas_relCirc",
...
df %>% select(all_of(cols_H1)) %>% select_if(is.numeric) %>% na.omit() %>% cor() %>% round(2) %>% 
  ggcorrplot::ggcorrplot(insig="blank", hc.order=T, lab=T, lab_size = 2, pch.cex = 1,p.mat = cor_pmat(.))
