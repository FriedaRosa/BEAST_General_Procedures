...
for (dd in seq_along(dfs_list)){
  ...
  model_df <- dfs_list[[dd]] %>% filter(complete.cases(.))
 ...
 if ("log_R2_1" %in% names(train)){
  ...
  caretList(
   ...
   }  else if ("Telfer_1_2" %in% names(train)) {
  ...
  models_list_all[[dd]] <- model_list
  

vip::vip(model_list[[1]], geom = "point", num_features = 25)
vip::vip(model_list[[3]], geom = "point", num_features = 25)


}
saveRDS(models_list_all, paste0(out_path, "rds/model_list_all.rds"))


models_list_all[[2]]$xgboost$results
...
for (i in seq_along(models_list_all)) {
  
dat <- models_list_all[[i]]$rf 


cols <- names(dat$trainingData)[c(-1)]

if ("Dist_toCOD" %in% cols) {
  cols <- cols[!names(cols) %in% "Dist_toCOD"]
}
...
for (y in seq_along(cols)){
  plot <- dat %>%  
    partial(pred.var = cols[y]) %>%
    plotPartial(smooth = TRUE, lwd = 2, ylab = "average predicted temporal change")
  plots_list1[[y]] <- plot
}
  })
plots_list2[[i]] <- plots_list1 

}


pdf(file = paste0(out_path, "pp.pdf"), onefile =T)
plots_list2
dev.off()
