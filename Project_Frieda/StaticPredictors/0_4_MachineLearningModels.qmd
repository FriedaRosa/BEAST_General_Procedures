...
```{r}
df <- readRDS(paste0(out_path, "rds/All_predictors.rds"))%>% unique() %>% 
  mutate_at(
    c("dataset", "tp", "grain","Habitat", "Migration", "Trophic.Level", "Trophic.Niche", "Primary.Lifestyle", "Habitat.Density", "Order", "Family"), as.factor) 
...
highly_correlated <- caret::findCorrelation(cor(df %>% na.omit() %>% select_if(is.numeric) %>% select(-atlas_xhalf, -atlas_yhalf), use="pairwise.complete.obs"), cutoff = 0.9, names = T)
