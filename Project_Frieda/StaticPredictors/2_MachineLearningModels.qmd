...
df <- readRDS("out/All_predictors.rds") %>% select(-p_value) %>% unique() %>% select(-neighbor_method) %>%   
  mutate_at(
    c("dataset", "years", "tp", "Order3", "grain","Habitat", "Migration", "Trophic.Level", "Trophic.Niche", "Primary.Lifestyle", "Habitat.Density"), as.factor) %>% rename("HWI" = "Hand-Wing.Index") 
rownames(df) <- NULL
...
df_red <- df %>% select(tp, dataset, verbatim_name, log_R2_1,
                                ...
                                global_moran,HWI, FP, Family3, Order3, PC1, PC2, 
                                Mass, Migration, Trophic.Level, Trophic.Niche, Habitat.Density, Range.Size, Primary.Lifestyle, 
                                AlphaSR_sp, BetaSR_sp, GammaSR, mean_prob_cooccur)%>% distinct(tp, dataset, verbatim_name, .keep_all = T)
...
```{r}
## Full Model: Future Change
m0_f <- randomForest(log_R2_1 ~ ., data=df_red1, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H1
m1_f <- randomForest(log_R2_1 ~ ., data=H1_Geometry_df1, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H2
m2_f <- randomForest(log_R2_1 ~ ., data=H2_Traits_df1, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H3
m3_f <- randomForest(log_R2_1 ~ ., data=H3_Diversity_df1, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 



## Full Model: Past Change
m0_p <- randomForest(log_R2_1 ~ ., data=df_red2, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H1
m1_p <- randomForest(log_R2_1 ~ ., data=H1_Geometry_df2, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H2
m2_p <- randomForest(log_R2_1 ~ ., data=H2_Traits_df2, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
# H3
m3_p <- randomForest(log_R2_1 ~ ., data=H3_Diversity_df2, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
...
sp.rf_ss <- randomForest(Telfer_1_2 ~ ., data=train_ss, ntree=50000, mtry=8, importance=TRUE, proximity=TRUE, na.action = na.omit, localImp = TRUE) 
sp.rf_ss 
