...
time_periods$tp <- c(1, 2,3, 2, 1, 1,2)
...
species_data <- occ_data_Atlas_all2 %>% select(-c(cell_label, area_sampled, area_c)) %>% distinct() 
...
for (y in seq_along(tp_dd_list)){
  ...
  for (b in seq_along(unique(temp_df$dataset))){
     temp_df <- temp_df %>% filter(dataset == unique(temp_df$dataset)[b])
     ...
     for (i in seq_along(unique(temp_df$verbatim_name))){
      ...
      # row.names = c("linear"),      
      verbatim_name = sp,
      dataset = unique(temp_df$dataset)[b],
      tp = y, 
...
species_data_new <- merge(species_data, 
                          all_atlas_list_allyears2, 
                          by=c(intersect(names(species_data), names(all_atlas_list_allyears2)))) %>% unique()
...
telfer_results_CZ <- sparta::telfer(taxa = CZ$verbatim_name,
                         site = CZ$cell_label,
                         time_period = CZ$tp,
                         minSite = 2)
telfer_results_CZ$dataset <- atlas_names[1]
telfer_results_NY <- sparta::telfer(taxa = NY$verbatim_name,
                         site = NY$cell_label,
                         time_period = NY$tp,
                         minSite = 2)
telfer_results_NY$dataset <- atlas_names[2]
telfer_results_JP <- sparta::telfer(taxa = JP$verbatim_name,
                         site = JP$cell_label,
                         time_period = JP$tp,
                         minSite = 2)
telfer_results_JP$dataset <- atlas_names[3]

telfer_res <- merge(telfer_results_CZ, telfer_results_NY, all =T)
telfer_res <- merge(telfer_res, telfer_results_JP, all =T)

telfer_results2 <- telfer_res %>% 
  select(taxa, Telfer_1_2, dataset) %>% 
  rename(verbatim_name = taxa)


species_data_new2 <- merge(species_data_new, telfer_results2, by=c("verbatim_name", "dataset"))
...
cor_df <- species_data_new3 %>% select("tp", "scale", "Total_area", "Total_Ncells", "occupancy_Ncells", "AOO", "relative_occupancy_Ncells", "m_AOO", "b_AOO", "D_AOO", "Telfer_1_2", "log_R2_1")
