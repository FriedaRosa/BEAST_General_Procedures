...
ggplot(dta) +
  geom_point(alpha = 0.4, aes(y = log_R2_1, x = value, color = tp)) +
  geom_smooth(aes(x = log_R2_1, y = value)) + 
  #labs(y = "", x = st_labs["log_R2_1"]) +
  scale_color_brewer(palette = "Set2") +
  facet_wrap(~ variable, scales = "free_x", ncol = 4) #+ scale_x_log10()



df$verbatim_name <- as.factor(df$verbatim_name)
...
df_red_clean <- df %>% select(-all_of(highly_correlated)) %>% distinct(tp, dataset, verbatim_name, .keep_all = T) #630 species
...
df_f$Family <-as.factor(df_f$Family)
df_f$Order <-as.factor(df_f$Order)

## Determine mtry & nodesize:
#o <- tune(log_R2_1 ~ ., df_f)
model <- rfsrc(log_R2_1 ~ ., data = df_f, ntree=1000, mtry = 35, nodesize = 2, na.action = "impute")
