...
for (i in seq_along(grids)){
  ...
  Geom_attributes_atlas <- data_frame(dataset = atlas_names[4],
  ...
  atlas_bbox$dataset <- atlas_names[4] 
...
presence_sf_list <- presence_sf_list0[4]
...
for (a in seq_along(presence_sf_list)){
  ...
  for (y in seq_along(time_periods)){
    ...
    for (s in seq_along(sp)){
      print(paste(atlas_names[4], time_periods[y], sp[s]))
