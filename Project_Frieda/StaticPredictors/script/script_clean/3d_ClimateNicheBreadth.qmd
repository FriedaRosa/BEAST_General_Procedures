...
# Atlas species names
spData <- read.csv("out/SpeciesNamesData.csv") %>% pull(x) 
...
# Lets continue with manual matchibg of names: (Note: Updated names were retrieved from Wikipedia on 21.04.2024)
...
Atlas_merged4 %>% filter(ScientificName %in% dups) %>% View()

name_vector <- Atlas_merged4 %>% pull(ScientificName)
Tax_LookUp <- Atlas_merged4 %>% distinct(AtlasSp, ScientificName)

write.csv(Tax_LookUp, "out/BirdLife2018_TaxonomicLookup.csv")
...
All_sp <- readRDS("out/rds/BOTW_All_species.rds")
...
sp_v <- stringr::str_split(list.files("data/species_rasters/"), 
                           pattern = ".tif")
...
pca_res$rotation %>% write.csv("documents/PCA_loadings_climNiche.csv")
...
## Add Atlas Species Names to Data
...
Niches_df <- plyr::rbind.fill(all_niches)[,1:3] %>% rename("ScientificName" = "species") %>%
    left_join(Tax_LookUp) %>% select(-ScientificName)
