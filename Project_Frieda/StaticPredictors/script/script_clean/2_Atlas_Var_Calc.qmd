...
big_table <- full_join(species_data_new2, logRatio) %>% 
    distinct(dataset, tp, cell_grouping, verbatim_name, .keep_all=T) %>%
...
# Save Output:
```{r}
saveRDS(occ_data_final, "SpeciesDatPerCell.rds") # data per cell
saveRDS(big_table, "BigTable.rds") # data per species
```