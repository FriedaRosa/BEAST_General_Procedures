...
pckgs <- c("dplyr", "ggplot2", "reshape2",  # Data shaping
           "caret", "ranger", "recipes") # Machine Learning
...
featurePlot(x = dat %>% select(-verbatim_name),
            y = dat$Jaccard, 
            group = dat$dataset,
            plot = 'pairs')
dev.off()
...
cor_df <- dat %>% select(-Jaccard, -verbatim_name)
...
set.seed(42)
trainIndex0 <- createDataPartition(dat_v2$Jaccard, p = 0.8, 1) # 10 resamples

# Initial split 
trainData <- dat_v2[trainIndex0[[1]], ] %>% select(-verbatim_name) # first resample
testData <- dat_v2[-trainIndex0[[1]], ] %>% select(-verbatim_name) # first resample
