---
title: "Report - Assignment for PhD Stats class 2024"
author: 
...
I will use the `caret` package for the regression-type machine learning models in combination with the `ranger()` function from the `ranger` package (setting `respect.unordered.factors = TRUE`) and `randomForest()` from the `randomForest` package for recursive feature selection.
...
pckgs <- c(
  "dplyr", "ggplot2", "reshape2", # Data shaping
  "caret", "ranger", "recipes", "randomForest", "glmnet", # Machine Learning
  "xtable", "styler", "lintr")  # Code/output formater
...
cols_atlas <- c(
  # "dataset",
  "Total_area_samp", "mean_area", "mean_cell_length", "atlas_lengthMinRect", "atlas_widthMinRect", "atlas_elonMinRect", "atlas_circ", "atlas_bearingMinRect", "atlas_bearing", "AtlasCOG_long", "AtlasCOG_lat"
)

setdiff(names(dat0), cols_atlas)
...
setdiff(names(dat0), c(cols_geo, cols_atlas))
...
# "IUCNLC", "IUCNEN",  "IUCNNT" , "IUCNVU",
  ...
  "Mass", # Body size
  "GlobRangeSize_m2", # Global range size (Breeding & Resident)
  "sd_PC1", "sd_PC2", # Global climate niche breadth
...
setdiff(names(dat0), c(cols_geo, cols_atlas, cols_trait))


# Diversity
cols_div <- c(
  "GammaSR", "AlphaSR_sp", "BetaSR_sp", # Gamma, Alpha and beta diversity
  ...
  "sp_centr_lon", "sp_centr_lat",# species distribution centroid coordinates)
  "mean_prob_cooccur"
  ) 


setdiff(names(dat0), c(cols_geo, cols_atlas, cols_trait, cols_div))
...
dat0 %>%
  ...
  theme_bw() +
  labs(
    title = "Responses, i.e. temporal change",
    x = "Value",
    y = "Frequency"
  )
...
dat0 %>%
  ...
  theme_bw() +
  labs(
    title = "Jaccard index of Similarity",
    x = "Value",
    y = "Frequency"
  )
...
dat %>%
  ...
  theme_bw() +
  labs(
    title = "Histograms of All Atlas Columns",
    x = "Value",
    y = "Frequency"
  )
...
dat %>%
  ...
  theme_bw() +
  labs(
    title = "Histograms of All Species Columns",
    x = "Value",
    y = "Frequency"
  )
```
...
```{r, all-features, fig.height = 100, fig.width = 100, eval = FALSE, include = TRUE}
...
```{r, visualization-raw3, fig.height = 20, fig.width= 22}
# Feature plots =====

feature_plots <- list(
  Atlas_Scale = featurePlot(
    x = dat[3:6],
    y = dat$Jaccard,
    group = dat$dataset,
    plot = "pairs",
  ...
  RangeDistance_Metrics = featurePlot(
    x = dat[46:49],
    y = dat$Jaccard,
    group = dat$dataset,
    plot = "pairs",
...
highly_correlated <- cor_matrix %>%
  filter(abs(Freq) > 0.7) # Adjust threshold as needed
highly_correlated

unique(highly_correlated$Var1)
unique(highly_correlated$Var2)
cor_vars <- intersect(highly_correlated$Var2, highly_correlated$Var1) %>% unique() %>% as.character()
...
The only near zero variable is IUCN status which is an important variable in the model, thus we will keep it.

The prepared recipes show that:

1.  the following 13 variables had pairwise pearson correlation coefficients \> 0.9:

    ```         
    "mean_cell_length"        "rel_ewDist"              "rel_nsDist"              "rel_circNorm"            "maxDist_toBorder_border" "maxDist_toBorder_centr"  "atlas_lengthMinRect"     "atlas_elonMinRect"       "AtlasCOG_long"           "mean_area"               "Total_area_samp"         "Total_Ncells_samp" "atlas_circ" 
    ```

2.  the following 6 variables were removed because of correlation coeficients \> 0.9 after atlas vars were combined to PC1, PC2, PC3.

    ```         
    "rel_ewDist"              "rel_nsDist"              "rel_circNorm"            "maxDist_toBorder_border" "maxDist_toBorder_centr"  "PC1" 
    ```
...
# With recipe:
# 1. Original recipe: imputation & correlation
pp_recipe1 <- recipe(Jaccard ~ .,
  data = dat
) %>%
...
# estimate recipe:
pp_recipe_prepped1 <- prep(pp_recipe1, dat)
pp_recipe_prepped1

# apply the recipe to the full dataset
dat_v2 <- bake(pp_recipe_prepped1, dat)
...
# 2. Let's try with combined correlated predictors (pca)
pp_recipe2 <- recipe(Jaccard ~ .,
  data = dat
) %>%
  step_impute_knn(all_predictors()) %>%
  step_normalize(all_of(cols_atlas)) %>%
  step_pca(all_of(cols_atlas), threshold = .95) %>%
  step_corr(all_numeric_predictors())

# estimate recipe:
pp_recipe_prepped2 <- prep(pp_recipe2, dat)
pp_recipe_prepped2$steps

# apply the recipe to the full dataset
dat_v3 <- bake(pp_recipe_prepped2, dat)

# remove old objects
rm(dat, nzv, pp_recipe1, pp_recipe2, pp_recipe_prepped1, pp_recipe_prepped2)
...
## Atlas predictors
dat_v2 %>%
  select(dataset, starts_with("atlas"), starts_with("Atlas")) %>%
  melt() %>%
  ggplot(aes(x = value, fill = dataset)) +
  geom_histogram(bins = 30, color = "black", alpha = 0.7) +
  facet_wrap(~variable, scales = "free_x") +
...
# Principle component axis 1 of correlated features
dat_v3 %>%
  select(PC2, PC3, dataset) %>%
  melt() %>%
  ggplot(aes(x = value, fill = dataset)) +
  geom_histogram(bins = 30, color = "black", alpha = 0.7) +
  facet_wrap(~variable, scales = "free_x") +
  labs(
    title = "Histograms of principle component axis 2 and 3 (atlas variables)",
    x = "Value",
    y = "Frequency"
  ) +
  scale_fill_manual(values = c("#0072B2", "#E69F00", "#009E73", "#D55E00")) +
...
set.seed(42)
trainIndex0 <- createDataPartition(dat_v2$Jaccard, p = 0.8, 1) # 0 resamples
trainIndexPCA <- createDataPartition(dat_v3$Jaccard, p = 0.8, 1) # 0 resamples


indices_single <- list(trainIndex0, trainIndexPCA)



# Initial split
# Raw data
trainData1 <- dat_v2[trainIndex0[[1]], ] # first resample
testData1 <- dat_v2[-trainIndex0[[1]], ] # first resample

# PCA data
trainData2 <- dat_v3[trainIndexPCA[[1]], ] # first resample
testData2 <- dat_v3[-trainIndexPCA[[1]], ] # first resample

train_data_single <- list(trainData1,  trainData2)
test_data_single <- list(testData1,  testData2)
...
# Raw Data
set.seed(42)
trainIndex1_ss <- createDataPartition(trainData1$Jaccard, 
                                      list = TRUE, 
                                      p = 0.8, 
                                      times = 10) # 10 resamples


# PCA data
set.seed(42)
trainIndex2_ss <- createDataPartition(trainData2$Jaccard, 
                                      list = TRUE, 
                                      p = 0.8, 
                                      times = 10) # 10 resamples


indices_resamples <- list(trainIndex1_ss, trainIndex2_ss)
```
...
```{r, Jacc-full-model0-single-split, eval = FALSE}

models_list <- list()
for (i in seq_along(1:2)){
  train_df <- train_data_single[[i]]
  
  # General Model Setup ========================================

  # Count predictors:
  max_pred <- model.matrix(
    ~ 0 + ., 
    data = train_df %>% select(-Jaccard)) %>%
    ncol()

  max_pred <- as.integer(max_pred / 2)
  max_pred # 21
  ...
  trainedControl <- trainControl(
    ...
    savePredictions = T,
    returnResamp = "all",
    index = indices_single[[i]][[i]], # 10 resamples
    verboseIter = TRUE)
...
model_rf0 <- train(
  Jaccard ~ .,
  data = train_df, # single split trainData
...
saveRDS(model_rf0, paste0("model_rf0" , "_", i, ".rds"))

models_list[[i]] <- model_rf0
}
...
```{r, model0-performance}
model_rf0 <- readRDS("model_rf0.rds")
...
pR2 <- ggplot(model_rf0, metric = "RMSE") +
  theme_bw()
pRMSE <- ggplot(model_rf0, metric = "Rsquared") +
  theme_bw()

# Variable importance:
pVarImp <- ggplot(varImp(model_rf0)) +
  theme_bw()
...
# In-bag Error:
postResample(
  pred = predict(model_rf0, newdata = trainData),
  obs = trainData$Jaccard
) # in-Bag-error: RMSE: 0.22, R2: 0.93
...
trainIndices <- list(trainIndex0, trainIndex1, trainIndex1b, trainIndex2b)
...
y_l <- list(y_nosplit1, y_split1,y_nosplit2, y_split2 )
x_l <- list(x_nosplit1, x_split1,x_nosplit2, x_split2)
...
```{r, recur-feature-selection-eval}
tictoc::tic()
