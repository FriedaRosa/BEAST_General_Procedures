...
```{r, fig.height = 100, fig.width = 100, eval = FALSE, include = TRUE}
...
highly_correlated <- cor_matrix %>%
  filter(abs(Freq) > 0.65) # Adjust threshold as needed
highly_correlated
...
dat_v3 %>%
  ...
  facet_wrap(~variable, scales = "free_x") +
  labs(
    title = "Histograms of PC1 (correlated variables)",
    x = "Value",
    y = "Frequency"
  ) +
  scale_fill_manual(values = c("#0072B2", "#E69F00", "#009E73", "#D55E00")) +
...
trainIndex1 <- createDataPartition(dat_v3$Jaccard, p = 0.8, 1) # 1 resamples
...
trainData2 <- dat_v3[trainIndex0[[1]], ] # first resample
testData2 <- dat_v3[-trainIndex0[[1]], ] # first resample

# Create 10 training (80%) and testing (20%) sets
set.seed(42)
trainIndex1_ss <- createDataPartition(trainData1$Jaccard, list = TRUE, p = 0.8, 10) # 10 resamples
set.seed(42)
trainIndex2_ss <- createDataPartition(trainData2$Jaccard, list = TRUE, p = 0.8, 10) # 10 resamples
```
...
max_pred <- model.matrix(
  ~ 0 + ., 
  data = trainData1 %>% select(-Jaccard)) %>%
  ncol()
...
trainedControl <- trainControl(
  ...
  savePredictions = T,
  returnResamp = "all",
  index = trainIndex1_ss, # 10 resamples
  verboseIter = TRUE)
...
model_rf0 <- train(
  Jaccard ~ .,
  data = trainData1, # single split trainData
...
saveRDS(model_rf0, "model_rf0.rds")
