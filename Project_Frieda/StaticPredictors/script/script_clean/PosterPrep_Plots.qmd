...
```{r}
 ...
 plot_dta <- sf2_list[[1]] %>% select(cell_label, verbatim_name, tp) %>% na.omit() %>% unique()
 
 CZ <- rnaturalearth::ne_countries(country = "czechia", scale = 10,  returnclass='sf') %>% st_crop(st_bbox(sf2_list[[1]]))
 
plot_dta %>% filter(tp == 1 & verbatim_name == "Corvus corax") %>% ungroup() %>%  group_by(verbatim_name) %>% summarise() %>% st_polygonize() %>%
   ggplot()+
   geom_sf(fill = "black")+
...
dta <- sf2_list[[1]] %>% 
   filter(tp == 1) %>% 
   select(cell_label, verbatim_name) %>% unique() %>%
   group_by(cell_label) %>%
   mutate(cell_alpha = n_distinct(verbatim_name)) %>% 
   select(cell_label, cell_alpha) %>% 
   unique()

 AlphaDiv <- dta %>%
   ggplot()+
      geom_sf(aes(fill = cell_alpha))+
      geom_sf(data = CZ, fill = NA)+
      theme_void()+
