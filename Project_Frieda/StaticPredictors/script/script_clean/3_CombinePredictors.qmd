...
BL_crosswalk <- readxl::read_excel("~/PhD_Projects/StaticPredictors/Data/BL_taxonomy_crosswalk.xlsx", sheet = 1, col_names = T) %>% select(1, 41)
...
fp <- phyloregion::evol_distinct(tree, type = "fair.proportion", scale = F, use.branch.lengths = T) %>% as.data.frame()
