...
atlas_names <- c("Birds_Atlas_Czechia", 
                 ...
                 "Birds_atlas_Japan", 
                 "Birds_atlas_EBBA")
df <- readRDS("../../out/rds/pres_dat_all.rds")


unique(df[[1]]$num_periods_cells) # reduced data


## Reduce to only EBBA data:
# df <- df[4]
# atlas_names <- atlas_names[4]
...
for (a in seq_along(atlas_names)){
    ...
    for (y in seq_along(time_periods)){
        ...
        for (s in seq_along(sp_list)){
            ...
            ## Credits: Dr. Carmen Diana Soria González de Buitrago  wrote the following code chunk to obtain presence-absence df for each species:
            data_species <- dd_y %>% 
...
# Save Output:
saveRDS(morans_list3, "../../out/rds/SAC_MoransList.rds")
saveRDS(data_df_all, "../../out/predictors/SAC_DataDf.rds")

save.image("../../out/RData/SAC.RData")
