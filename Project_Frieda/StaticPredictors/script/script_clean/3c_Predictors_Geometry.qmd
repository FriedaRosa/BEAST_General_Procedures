...
COG_all <- list()
COG_a <- list()
presence_sf_list0 <- readRDS("presence_sf_list.rds")
  ...
  atlas <- presence_sf_list[[a]] %>% 
    ...
    for (s in seq_along(sp)){
      ...
      species_df <- atlas_sp %>% 
       ...
       Dist_centroid_to_COG <- distance(crds(points_species), crds(points_atlas), lonlat = T)
       minDist_toBorder_centr <- min(distance(crds(points_species), crds(atlas_lines), lonlat=T))
       maxDist_toBorder_centr <- max(distance(crds(points_species), crds(atlas_lines), lonlat=T))
       minDist_toBorder_border <- min(distance(crds(species_df), crds(atlas_lines), lonlat=T))
       maxDist_toBorder_border <- max(distance(crds(species_df), crds(atlas_lines), lonlat=T))
       
       Geom_attributes <- data_frame(dataset = atlas_names[a],
                                    tp = time_periods[y],
                                    verbatim_name = sp[s],
                                    nsDist = poly_attr(species_df, "nsDist"),
                                    ewDist = poly_attr(species_df, "ewDist"),
                                    maxDist = poly_attr(species_df, "maxDist"),
                                    lengthMinRect = poly_attr(species_df, "lengthMinRect"),
                                    widthMinRect = poly_attr(species_df, "widthMinRect"),
                                    elonMinRect = poly_attr(species_df, "elonMinRect"),
                                    elonRatio = poly_attr(species_df, "elonRatio"),
                                    circ = poly_attr(species_df, "circ"),
                                    circNorm = poly_attr(species_df, "circNorm"),
                                    relCirc = poly_attr(species_df, "relCirc"),
                                    lin = poly_attr(species_df, "lin"),
                                    bearingMinRect = poly_attr(species_df, "bearingMinRect"), 
                                    bearing = poly_attr(species_df, "bearing"),
      ...
      Geom_attributes2 <- cbind(Geom_attributes, crds(points_species))
...
COG_df <- plyr::rbind.fill(COG_a, fill =T) %>% rename("Atlas_COG_lon" = "x", "Atlas_COG_lat" = "y")
Species_geom_attributes <- plyr::rbind.fill(atlas_all_sp_geom) %>% rename("sp_centr_lon" = "x", "sp_centr_lat" = "y")

Atlas_geom <- full_join(countries_geom_attributes, COG_df) %>% 
    group_by(dataset) %>% 
...
saveRDS(Geometries_final2, paste0(out_path, "rds/Geometries.rds"))
saveRDS(Geometries_final2, "Geometries.rds")
save.image("Geometry.RData")
tictoc::toc()
```

# This still produces MANY MANY NAs
colSums(is.na(Geometries_final2))
                dataset                      tp           verbatim_name
                      0                       4                       4
          lengthMinRect            widthMinRect             elonMinRect
                      4                       4                       4
                   circ          bearingMinRect                 bearing
                      4                       4                       4
   Dist_centroid_to_COG minDist_toBorder_border maxDist_toBorder_border
                      4                       4                       4
 minDist_toBorder_centr  maxDist_toBorder_centr                       x
                      4                       4                       0
                      y              atlas_xmin              atlas_xmax
                      0                    2154                    2154
            atlas_xhalf              atlas_ymin              atlas_ymax
                   2154                    2154                    2154
            atlas_yhalf     atlas_lengthMinRect      atlas_widthMinRect
                   2154                    2154                    2154
      atlas_elonMinRect              atlas_circ    atlas_bearingMinRect 
                   2154                    2154                    2154
          atlas_bearing            Southernness             Westernness
                   2154                    2154                    2154
