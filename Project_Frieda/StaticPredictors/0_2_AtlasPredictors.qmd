...
```
```{r}
load("out/RData/AtlasPredictors_1.RData")
```
...
tax_lookup <- read.csv2(paste0(out_path, "csv/Tax_lookup_BirdLife_eBird_BirdTree_AtlasBOTW.csv"))
...
sp_df_big <- big_tab %>% 
  ...
  left_join(RangeSizeNew %>% rename("sp_BirdLife" = "sci_name"))%>% 
  filter_at(vars(log_R2_1), any_vars(!is.na(.))) %>% 
  full_join(Niches_df %>% rename("sp_BirdLife" = "verbatim_name")) %>% 
  select(-sp_atlas_adapted) %>% unique()
...
```{r}
# Save Environment until here
load(paste0(out_path, "RData/AtlasPredictors_1.RData"))
...
avonet_final %>% filter(is.na(sp_atlas.adapted1))
avonet_final 
names(BOTW_RL)
...
avonet_coalesced <- avonet_final2 %>% mutate(
...
avonet_final %>% filter_all(any_vars(is.na(.))) # 6 Species without range size (and other data); 101 species without red list
avonet_final %>% filter_all(any_vars(!is.na(.))) %>% distinct(verbatim_name) %>% nrow() #841 species have data
...
predictors_df3 <- full_join(Geometries, predictors_df2) 

# Co-Occurrence index
Co_Occ_df_final <- readRDS(paste0(out_path, "rds/Coocc_df_final.rds"))
predictors_df4 <- full_join(Co_Occ_df_final, predictors_df3) 

# Avonet
predictors_df5 <- left_join(predictors_df4, avonet_final) %>% 
  distinct(verbatim_name, tp, dataset, .keep_all=T) 

# All vars NA for  ? species:
predictors_df5 %>% filter_all(all_vars(!is.na(.))) %>% 
  distinct(verbatim_name, dataset, tp) %>% 
  group_by(dataset, tp) %>% 
  summarise(n=n())

# Evol. distinctness
predictors_df5$tip.label <- gsub(" ", "_", predictors_df5$sp_BirdTree)
setdiff(predictors_df5$tip.label, FP$tip.label) %>% length()

predictors_df6 <- left_join(predictors_df5, FP) %>% 
...
predictors_df7 <- full_join(predictors_df6, SAC_df); 
```
