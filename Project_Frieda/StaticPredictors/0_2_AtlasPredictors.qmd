...
# SAC:
# library(spdep); library(sfdep) # old SAC script
...
for (a in seq_along(atlas_names)){
  ...
  for (y in seq_along(time_periods)){
    ...
    for (s in seq_along(sp_list)){  
      ...
      data_species <- dd1 %>% 
        select(verbatim_name, cell_label, cell_lat, cell_long) %>% 
        ungroup() %>% unique() %>%
        mutate(
      ...
      data_reduced <- dd1 %>% 
        ...
        mutate(
          increment2 = mean_cell_length * 1.25) %>% 
        ungroup() %>% 
      ...
      tryCatch({
         ...
         morans_cor <- correlog(x = data_species$cell_long, y = data_species$cell_lat, z = data_species$presence, 
                                latlon=T, increment = unique(data_species$increment), resamp=0)
         morans_list[[s]] <- morans_cor 

         data_reduced$moran <- as.numeric(morans_cor$correlation[1])
         data_reduced$x_intercept <- morans_cor$x.intercept
        
      }, error = function(e) {
        # Handle the case where Moran's I cannot be calculated
         data_reduced$moran <- "Error"
         data_reduced$x_intercept <- "Error"
         morans_list[[s]] <- "Error"
      })
    ...
    data_list3[[y]] <- data_df2
    } # atlas loop closing

data_df_all <- plyr::rbind.fill(data_list3)
saveRDS(morans_list3, paste0(out_path, "rds/morans_list.rds"))

SAC_df_ncf <- plyr::rbind.fill(data_df_all, fill = T) %>% unique()
...
write.csv(SAC_df, paste0(out_path, "csv/SAC_data.csv"))
saveRDS(SAC_df, paste0(out_path, "rds/SAC_data.rds"))
...
SAC_df <- readRDS(paste0(out_path, "rds/SAC_data.rds"))
