...
predictors_df3 <- full_join(predictors_df2, AvgEffort)
...
### SAC: Distance-based (WIP)

```{r, eval = F, include = F}

sf_use_s2(FALSE)
out_list_distance <- list()
time_periods <- c(1,2)
atlas_names <- c("Birds_Atlas_Czechia", "Birds_Atlas_New_York","Birds_atlas_Japan", "Birds_atlas_EBBA")
presence_sf_list <- list()
    ...
    for (s in seq_along(sp_list)) {
      ...
      data_species <- dd1 %>%
       ...
       tryCatch({
         ...
         coords <- st_coordinates(st_centroid(data_species)) %>% unique()
         knear <- knn2nb(knearneigh(coords, k=1))
         dsts<-unlist(nbdists(knear, coords, longlat = F))
         max(dsts) #max distance necessary to connect to neighbors
         
         #Neighbour defined by dnearneigh()
         nb<-dnearneigh(coords,0,max(dsts), longlat=F) #To match Africa dataset
         par(mfrow=c(3,4), mar=c(2,4,1,1))
         plot(coords)
         plot(nb, coords, pch=20, lwd=2)
         summary(nb)
         
         nb_100<-dnearneigh(coords,0,100, longlat=T)
         summary(nb_100)
         plot(nb_100, coords, pch=20, lwd=2)
         
         nb1_w<-nb2listw(nb, glist=NULL, style="W", zero.policy=T)
...
sign.mc <- ifelse(sign.mc > 0.5, 1 - sign.mc, sign.mc)
    ...
    out_list_distance[[length(out_list_distance) + 1]] <- spmoran_df
...
predictors_df4 <- full_join(predictors_df3, SAC_df)
predictors_df4
...
```{r}
all_predictors0 <- full_join(predictors_df6, atlas_predictors_df2)
str(all_predictors0)

all_predictors <- all_predictors0 %>%
  select(-neighbor_method, -verbatim_name) %>%
  mutate_at(
    c("dataset", "years", "tp"), as.factor
  )
