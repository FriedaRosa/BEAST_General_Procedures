...
for (i in seq_along(grids)){
  ...
  Geom_attributes <- data_frame(dataset = atlas_names[i],
           nsDist = poly_attr(Atlas_Geom, "nsDist"),
           ewDist = poly_attr(Atlas_Geom, "ewDist"),
           maxDist = poly_attr(Atlas_Geom, "maxDist"),
           lengthMinRect = poly_attr(Atlas_Geom, "lengthMinRect"),
           widthMinRect = poly_attr(Atlas_Geom, "widthMinRect"),
           elonMinRect = poly_attr(Atlas_Geom, "elonMinRect"),
           elonRatio = poly_attr(Atlas_Geom, "elonRatio"),
           circ = poly_attr(Atlas_Geom, "circ"),
           circNorm = poly_attr(Atlas_Geom, "circNorm"),
           relCirc = poly_attr(Atlas_Geom, "relCirc"),
           lin = poly_attr(Atlas_Geom, "lin"),
           bearingMinRect = poly_attr(Atlas_Geom, "bearingMinRect"), 
           bearing = poly_attr(Atlas_Geom, "bearing"))
...
```{r}
Geometries <- readRDS(paste0(out_path, "Geometries.rds")) %>% na.omit()
grids # Atlases
atlas_names
time_periods
...
# Transform Geometry lat/long from CoG to points
points_COG_atlas <- Geometries %>% 
  group_by(dataset, tp) %>% 
  dplyr::select(CenterOfGravity_Atlas_long, CenterOfGravity_Atlas_lat, dataset, tp) %>%
  rename("x" = "CenterOfGravity_Atlas_long", "y" = "CenterOfGravity_Atlas_lat") %>% na.omit() %>% distinct()

points_COG_sp <- Geometries %>% 
  group_by(dataset, tp, verbatim_name) %>% 
  dplyr::select(sp_COG_long, sp_COG_lat, dataset, tp, verbatim_name) %>%
  rename("x" = "sp_COG_long", "y" = "sp_COG_lat") %>% na.omit() %>% ungroup()

### Calculate Distances =========================================
sp <- unique(points_COG_sp$verbatim_name)
times <- as.numeric(c(1,2))
atlas <- unique(points_COG_sp$dataset)
...
for (a in seq_along(atlas_names)) {
  ...
  df1_atlas <- points_COG_atlas %>%
    filter(dataset == atlas_names[a])


  for (y in seq_along(time_periods)) {
    df_tp1 <- df1 %>% filter(tp == 1)
    df_tp2 <- df1 %>% filter(tp == 2)
    dfs <- list(df_tp1, df_tp2)

    df2_atlas1 <- df1_atlas %>% filter(tp == 1)
    df2_atlas2 <- df1_atlas %>% filter(tp == 2)
    dfs_atlas <- list(df2_atlas1, df2_atlas2)


    sp <- unique(dfs[[y]]$verbatim_name)


    for (s in seq_along(sp)) {
      temp_df <- dfs[[y]] %>%
        filter(verbatim_name == sp[i])

      points2 <- temp_df %>%
        vect(geom = c("x", "y"), crs = "epsg:4326")

      points_atlas <- dfs_atlas[[y]] %>%
        vect(geom = c("x", "y"), crs = "epsg:4326")


      ## Calculate distance
      atlas_lines <- lines_l[[a]]
      minDist_toBorder <- min(terra::distance(crds(atlas_lines), crds(points2), lonlat = T))
      Dist_toCOG <- terra::distance(crds(points2), crds(points_atlas), lonlat = T)
      ...
      Distance_df <- data.frame(
        ...
        tp = time_periods[y],
        minDist_toBorder = minDist_toBorder,
        Dist_toCOG = Dist_toCOG
      )

      dist_list_sp[[s]] <- Distance_df
    }

    dist_df_sp <- plyr::rbind.fill(dist_list_sp)
    dist_list_y[[y]] <- dist_df_sp
  }
  dist_df_y <- plyr::rbind.fill(dist_list_y)
  dist_list_a[[a]] <- dist_df_y
}

dist_df_a <- plyr::rbind.fill(dist_list_a)

table(dist_df_a$dataset)

Geometries_final <- full_join(Geometries, dist_df_a) %>% distinct()
