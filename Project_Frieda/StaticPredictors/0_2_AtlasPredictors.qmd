...
Predictors_df <- readRDS(paste0(out_path, "all_predictors_niches.rds"))
```
...
predictors_df7 <- left_join(predictors_df6, avonet) %>% unique()
predictors_df7
...
predictors_df8 <- left_join(predictors_df7, FP) %>% select(-tip.label)
```
...
all_predictors0 <- unique(all_predictors0)
...
all_predictors <- all_predictors0 %>% rename("atlas_Total_area" = "Total_area") %>%
  ...
  mutate_at(
    c("dataset", "years", "tp", "Order3", "grain", "Family3", "Habitat", "Migration", "Trophic.Level", "Trophic.Niche", "Primary.Lifestyle", "Habitat.Density"), as.factor
  ) %>% 
...
model.matrix(~0+., data=cor_df) %>% 
  ...
  ggcorrplot(tl.cex = 5, type = "upper",outline.color = "white")
...
save.image("~/GitHub/BEAST_General_Procedures/Project_Frieda/StaticPredictors/out/Atlas_predictors_all_Server.RData")
load("~/GitHub/BEAST_General_Procedures/Project_Frieda/StaticPredictors/out/Atlas_predictors_all_Server.RData")
```
