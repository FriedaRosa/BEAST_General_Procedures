...
sp_df_big <- big_tab %>% 
  ...
  left_join(tax_lookup %>% distinct(verbatim_name, sp_names_atlas_adapted_keep ,sp_BirdTree, sp_BirdLife)) %>%
...
avonet_final <- avonet_coalesced[,c(1:2, 26:37)] %>% distinct(sp_atlas, .keep_all=T) %>% rename("verbatim_name" = "sp_atlas")
...
FP2 <- right_join(FP %>% rename("sp_BirdTree" = "verbatim_name"), tax_lookup) %>% 
  select(verbatim_name, FP)
predictors_df6 <- left_join(predictors_df5, FP2) %>% 
  select(-sp_BirdLife, -sp_BirdTree)
