...
occ_data_red <- occ_data_full_df2 %>% select(-c(cell_label, area_sampled, area_c)) %>% distinct() 
# occ_data_red %>% write.csv(paste0(out_path, "Occupancy_table.csv"))
```
...
occ_data_red2 <- merge(occ_data_red, m_df_wide_AOO, 
                          by=c("tp", "verbatim_name"), all = T) %>% distinct()
occ_data_red3 <- merge(occ_data_red2, telfer_results2, 
...
low_occu_sp_sample <- occ_data_red3 %>% 
   filter(verbatim_name %in% c(sample(low_occu_sp, 9))) %>% 
   pull(verbatim_name) %>% 
   unique()

low_occu_reduced <- occ_data_red3 %>% 
  filter(cell_grouping == "cell1grid") %>% 
  filter(verbatim_name %in% low_occu_sp) %>%
  select(verbatim_name, relative_occupancy_area, tp) %>%
  distinct() %>% 
...
high_occu_sp_sample <- occ_data_red3 %>% 
...
model_df1<- occ_data_red3 %>% 
  select(verbatim_name, tp, AOO, scale, cell_grouping) %>%
  filter(verbatim_name == "Grus grus") %>% 
  #filter(cell_grouping %in% s_linear) %>%
  filter(tp == 1) %>% 
  unique() 
model_df2<- occ_data_red3 %>% 
  select(verbatim_name, tp, AOO, scale, cell_grouping) %>%
  filter(verbatim_name == "Grus grus") %>% 
  #filter(cell_grouping %in% s_linear) %>%
  filter(tp == 2) %>% 
  unique() 
model_df3<- occ_data_red3 %>% 
  select(verbatim_name, tp, AOO, scale, cell_grouping) %>%
  filter(verbatim_name == "Grus grus") %>% 
  #filter(cell_grouping %in% s_linear) %>%
  filter(tp == 3) %>% 
...
```{r}
occ_data_red3 %>%  filter(cell_grouping == "cell1grid") %>%
ggplot() +
  geom_boxplot(aes(y = relative_occupancy_area, fill = factor(tp))) + scale_color_viridis(discrete = T)

                                  
cols_n <- intersect(names(presence_data), names(occ_data_red3))
occ_data_full <- unique(merge(presence_data, occ_data_red3, 
                              by = cols_n, 
                              all.y = T))
...
# D ~ Telfer Plot ==================================== ####
occ_data_red4 <- occ_data_red3
occ_data_red4$D <- c(-occ_data_red4$m)
occ_data_red4 %>% filter(tp == 1) %>%
ggplot() +
  geom_point(aes(x = Telfer_1_2, y = D ))+
  theme_classic()

occ_data_red4 %>% filter(tp == 2) %>%
ggplot() +
  geom_point(aes(x = Telfer_2_3, y = D ))+
  theme_classic()

occ_data_red4 %>% filter(tp == 3) %>%
ggplot() +
...
occ_data_red_wide1 <- occ_data_red4 %>% 
...
occ_data_red_wide2 <- occ_data_red4 %>% 
  select(verbatim_name, tp, cell_grouping, 
         D, b, AOO, Total_Ncells, 
         occupancy_area, occupancy_Ncells, 
         relative_occupancy_Ncells, relative_occupancy_area) %>% 
...
occ_data_red_wide3 <- occ_data_red4 %>% 
  select(verbatim_name, tp, cell_grouping, 
         D, b, AOO, Total_Ncells, 
         occupancy_area, occupancy_Ncells, 
         relative_occupancy_Ncells, relative_occupancy_area) %>% 
...
# merge back together:
temp <- merge(occ_data_red_wide1, occ_data_red_wide2, 
              by=intersect(names(occ_data_red_wide1), names(occ_data_red_wide2)))
temp2 <- merge(temp, occ_data_red_wide3,
               by=intersect(names(temp), names(occ_data_red_wide3)))
names_v <- names(temp2[-(1:2)])

# Transform to wide format by cell_grouping
occ_data_red_wide <- temp2 %>% 
  pivot_wider(names_from = cell_grouping,
              values_from = all_of(names_v))

occ_data_red_wide <- occ_data_red_wide %>% 
